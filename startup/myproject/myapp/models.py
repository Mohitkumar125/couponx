...
class Register(models.Model):
    ...
    # Any additional registration-specific fields can be added here
...
class Coupon(models.Model):
    ...
    prize = models.ForeignKey(Prize, on_delete=models.CASCADE, null=True, blank=True)  # Optional relation
...
class CustomerWinner(models.Model):
    ...
    def __str__(self):
        ...
        return f"{self.customer_name} - {prize_name}"


class UserProfile(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    package_active = models.BooleanField(default=False)  # Indicates if package is activated by admin
    plan_expiration_date = models.DateField(null=True, blank=True)  # Expiration date

    def is_package_active(self):
        """Return True if package is active (enabled and not expired)"""
        today = date.today()
